#!/usr/bin/env python3
"""
Evaluate semantic rendering with alpha-normalization (debug only).

Idea:
- Render semantic logits as usual (sparse).
- Render a constant signal with cls_ids forced to 0 to approximate per-pixel weight sum.
- Normalize semantic logits by that weight sum and compute metrics.
"""
...
# Needed for third_parties.splatam relative imports inside eval_helper.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate semantic rendering with alpha-normalization")
    ...
    parser.add_argument("--out_suffix", type=str, default="final_alpha_norm", help="Eval dir suffix")
...
def main() -> None:
    ...
    n_cls = slam.n_cls
    topk = slam.params["semantic_logits"].shape[1]
    ...
    for time_idx in range(len(dataset)):
        ...
        # Render weight sum approximation: all cls_ids=0 and colors_precomp=ones
        cls_zero = torch.zeros_like(cls_ids)
        cam_zero = set_camera_sparse(cam=cam, cls_ids=cls_zero)
        rendervar_const = dict(seman_rendervar)
        rendervar_const["colors_precomp"] = torch.ones_like(seman_rendervar["colors_precomp"])
        sem_zero, _ = SEMRenderer(raster_settings=cam_zero)(**rendervar_const)

        # Weight sum per pixel (class 0 channel accumulates K contributions)
        weight_sum = sem_zero[0] / float(topk)
        weight_sum = torch.clamp(weight_sum, min=1e-6)

        # Alpha-normalize semantic logits
        rastered_norm = rastered_seman / weight_sum.unsqueeze(0)
        rastered_norm = torch.nan_to_num(rastered_norm, nan=0.0)
        rastered_norm[rastered_norm < 0] = 0.0

        # Metrics
        pred_logits = rastered_norm.permute(1, 2, 0)
    ...
    print(f"[eval_alpha_norm] wrote {out_path}")
