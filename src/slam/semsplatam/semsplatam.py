...
class SemSplatam(SplatamOurs):
    def __init__(self,
       ...
       '''
        ...
        # Cache class id -> name mapping (include unknown=0)
        self.class_id_to_name = {0: "unknown"}
        try:
            with open(self.class_info_file, "r") as f:
                class_info = json.load(f)
            for k, v in class_info.items():
                self.class_id_to_name[int(k)] = v.get("name", "unknown")
        except Exception:
            pass
        ...
        self.debug_helper = dbg.SemanticDebugHelper(
            result_dir=self.main_cfg.dirs.result_dir,
            n_cls=self.n_cls,
    ...
    def _can_eval_semantic(self, dataset) -> bool:
        """Return True if semantic GT is available for eval.

        NOTE: Some datasets (e.g., replica_sim_nvs generated by generate_Replica_NVS_data.py)
        do not include semantic GT files. In that case, eval_semantic must be skipped to avoid
        IndexError in dataset.get_semantic_map. To revert, either provide semantic GT files
        in dataset_eval_basedir or remove this guard once semantic data exists.
        """
        if dataset is None:
            return False
        if not getattr(dataset, "load_semantics", False):
            return False
        semantic_paths = getattr(dataset, "semantic_paths", None)
    ...
    def semantic_annotation(self, input_img: torch.Tensor):
        ...
        if len(class_logits_from_oneformer) > 0:
            self.debug_helper.log_oneformer_stats(
                class_logits_from_oneformer[0],
                self.info_printer,
                self.step,
    ...
    def init_camera_parameters(self):
        ...
        # ACTIVE_SGM_DEBUG_SEMAN_TRANSFER_SYNC=1 syncs CUDA before transfer for determinism.
        if os.environ.get("ACTIVE_SGM_DEBUG_SEMAN_TRANSFER_SYNC", "") == "1" and seman.is_cuda:
            try:
                torch.cuda.synchronize(seman.device)
            except Exception:
                pass
        # ACTIVE_SGM_SEMAN_CPU_HOP=1 forces CPU hop to avoid GPU P2P corruption.
        if os.environ.get("ACTIVE_SGM_SEMAN_CPU_HOP", "") in ("1", "true", "True"):
            # Optional CPU hop to avoid GPU peer-to-peer copy corruption in multi-GPU setups.
            # Leave off for single-GPU runs (default).
            seman_cpu = seman.detach().to("cpu")
            seman_dst = seman_cpu.to(self.device)
        else:
            seman_dst = seman.to(self.device)
        if os.environ.get("ACTIVE_SGM_DEBUG_SEMAN_TRANSFER", "") in ("1", "true", "True"):
            self.debug_helper.log_seman_transfer(seman, seman_dst, "init_seman_to_device", -1)
        seman = seman_dst
    ...
    def render_semantic(self, c2w: torch.Tensor, seen: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        self.debug_helper.log_render_sem_inputs(
            seen,
            seman_rendervar,
            self.info_printer,
            self.step,
        ...
        # Align cls_ids with seen Gaussians to avoid class/gaussian index mismatch.
    ...
    def online_recon_step(self, 
        ...
        class_ids, seman = self.semantic_annotation(seg_img)
        self.debug_helper.save_semantic_debug(time_idx, color, class_ids, seman)
        self.debug_helper.maybe_sync_seman(seman, "post-annotation")
    ...
    def update_gs_map(self, 
        ...
        depth = depth.to(self.device)
        self.debug_helper.log_seman_stats(seman, "seman_input_hwc", time_idx)
        self.debug_helper.log_isfinite(seman, "seman_input_hwc", time_idx)
        seman = seman.permute(2, 0, 1)  # C,H,W
        self.debug_helper.log_isfinite(seman, "seman_perm", time_idx)
        self.debug_helper.log_seman_stats(seman, "seman_perm_chw", time_idx)
        # ACTIVE_SGM_DEBUG_SEMAN_TRANSFER_SYNC=1 syncs CUDA before transfer for determinism.
        if os.environ.get("ACTIVE_SGM_DEBUG_SEMAN_TRANSFER_SYNC", "") == "1" and seman.is_cuda:
            try:
                torch.cuda.synchronize(seman.device)
        ...
        if os.environ.get("ACTIVE_SGM_SEMAN_CPU_HOP", "") in ("1", "true", "True"):
            ...
            seman_dst = seman.to(self.device)
        if os.environ.get("ACTIVE_SGM_DEBUG_SEMAN_TRANSFER", "") in ("1", "true", "True"):
            self.debug_helper.log_seman_transfer(seman, seman_dst, "seman_perm_to_device", time_idx)
        seman = seman_dst
        self.debug_helper.log_isfinite(seman, "seman_to_dev", time_idx)
        self.debug_helper.log_seman_stats(seman, "seman_chw", time_idx)
        self.debug_helper.log_contig_check(seman, time_idx)
        self.debug_helper.maybe_sync_seman(seman, "update_gs_map")
        self.debug_helper.maybe_cpu_check(seman)
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0 or force_map_update:
            ...
            for iter in range(num_iters_mapping):
                ...
                debug_param_sem = self.debug_helper.param_sem_debug_enabled()
                if debug_param_sem and iter == 0:
                    self.debug_helper.log_param_sem(
                        params,
                        "before_loss",
                        iter,
                        self.info_printer,
                ...
                with torch.no_grad():
                    ...
                    if debug_param_sem:
                        self.debug_helper.log_param_sem_nonfinite(
                            params,
                            iter,
                            self.info_printer,
                            self.step,
    ...
    def print_and_save_result(self, eval_dir_suffix="", is_prune_gaussians=False, ignore_first_frame=False):
        ...
        with torch.no_grad():
            if config['use_wandb']:
                ...
                if self._can_eval_semantic(dataset):
                    ...
                    print("[ActiveSGM] Skipping eval_semantic: semantic GT not available in dataset_eval.")
            else:
                eval(self,dataset, params,variables, len(dataset), eval_dir,sil_thres=config['mapping']['sil_thres'],
                    mapping_iters=config['mapping']['num_iters'], add_new_gaussians=config['mapping']['add_new_gaussians'],
                    eval_every=config['eval_every'],
